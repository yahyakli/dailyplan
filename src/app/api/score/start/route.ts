...
export async function POST(req: NextRequest) {
  try {
    ...
    // Update streak based on the plan's date being started today
    // To simplify, we'll mark today as the last active day
    ...
    // Calculate plan creation bonus (stripped down version of calculatePoints)
    let pointsEarned = 10 // Base points for starting plan
    const breakdown = [{ reason: 'Started your plan', points: 10 }]

    if (score.allTimePlans === 0) {
      pointsEarned += 20
      breakdown.push({ reason: 'Welcome bonus — first plan started!', points: 20 })
    }

    // Check for start-related badges
